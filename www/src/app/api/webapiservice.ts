...
export class Client {
    ...
    // Hand-added to match the "GetMyListings" backend endpoint; regenerate via nswag once deployed to drop this note.
    /**
     * @return OK
     */
    getMyListings(): Promise<GetListingsOutput> {
        let url_ = this.baseUrl + "/listings/mine";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetMyListings(_response);
        });
    }

    protected processGetMyListings(response: Response): Promise<GetListingsOutput> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
